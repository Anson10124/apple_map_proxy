...
pub fn composite_hybrid_tile(sat_bytes: &[u8], overlay_bytes: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let overlay_rgba: RgbaImage = if sat_w != ov_w || sat_h != ov_h {
        let resized = overlay_img.resize_exact(sat_w, sat_h, imageops::FilterType::Lanczos3);
        resized.to_rgba8()
    } else {
